...
// Add these includes at the top, before using filesystem
#include <filesystem>
#include <string>
...
// Add namespace declarations after includes
using std::string;
...
static void my_application_activate(GApplication* application) {
  ...
  const string iconFilename = "assets/logo.png";
  ...
  // Thiết lập màu nền cho cửa sổ (màu xanh #3498DB)
  GdkRGBA color;
  gdk_rgba_parse(&color, "#3498DB");

  // Tạo CSS provider để áp dụng màu nền
  GtkCssProvider *provider = gtk_css_provider_new();
  gtk_css_provider_load_from_data(provider,
                                 "window { background-color: #3498DB; }", -1, NULL);
  ...
  // Hiển thị cửa sổ sau khi đăng ký plugin
